...
ARG USER_GID=1000
RUN apt-get update \
  && apt-get install -y --no-install-recommends ca-certificates gosu curl git wget ripgrep python3 build-essential python3-pip python3-venv \
...
RUN npm install --global --omit=dev @anthropic-ai/claude-code@latest @openai/codex@latest opencode-ai vercel \
  ...
  && mkdir -p /paperclip /paperclip/hermes/.hermes \
